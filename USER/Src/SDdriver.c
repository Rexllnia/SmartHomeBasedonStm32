...
void SD_CS(uint8_t p){
	if(p==0){	
		...
		HAL_GPIO_WritePin(SDcard_CS_PORT,SDcard_CS_PIN,GPIO_PIN_RESET);
		}
}
///////////////////////////////////////////////////////////////
//发送命令，发完释放
...
uint8_t spi_readwrite(uint8_t Txdata){
	uint8_t Rxdata;	
	HAL_SPI_TransmitReceive(&SDcard_hspi,&Txdata,&Rxdata,1,100);
//	UART1_printf("%d",Rxdata);
	return Rxdata;
}
//SPI3波特率设置
void SPI_setspeed(uint8_t speed){
	SDcard_hspi.Init.BaudRatePrescaler = speed;
}
